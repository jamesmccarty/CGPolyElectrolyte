...
c modified for charge types

      program chain

      integer swaptype,counter,Nmono
      ...
      real*8, allocatable :: x(:),y(:),z(:),charge(:)
      real*8, allocatable :: bondlength(:),restrict(:)
      integer seqlength
      integer salttypes,atomtypes

      CHARACTER(LEN=:), allocatable :: sequenceid
 ...
 901  format(2f15.6,a)
 902  format(i3,f5.1)
 903  format(i10,i8,i8,4f10.4,3i4)
...
!     read (5,*) swaptype
      ...
      do iset = 1,nsets
        ...
        ntype(iset) = 1 ! default monomer type
        nbondtype(iset) = 1 ! default bond type
        read (5,*) bondlength(iset)
        read (5,*) restrict(iset)
      enddo
      read(5,*) salttypes
      seqlength=nmonomer(1)
      allocate(CHARACTER(LEN=seqlength) :: sequenceid)
      read(5,*)sequenceid
...
c write out LAMMPS file
      counter = 1
      Nmono = natoms/nmolecule
      do i = 1,nmolecule
         Do j = 1,seqlength
          IF(sequenceid(j:j)=='A') then
             type(counter) = 1
             charge(counter) = -1.0
             counter = counter + 1
          END IF
          IF(sequenceid(j:j)=='C') then
              type(counter) = 2
              charge(counter) = 1.0
              counter = counter +1
          END IF
        END DO
      enddo
      write (6,900) 'LAMMPS chain data file'
      write (6,*)
      ...
      write (6,*) 0,' impropers'
      write (6,*)
      atomtypes=2+salttypes
      write (6,*) atomtypes,'  atom types'
...
c      enddo
      ...
      do i = 1,natoms
        write (6,903) i,molecule(i),type(i),charge(i),x(i),y(i),z(i),
